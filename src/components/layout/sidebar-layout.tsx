...
export function SidebarLayout({ children, sidebar, header }: SidebarLayoutProps) {
  return (
    <div className="grid grid-cols-[440px_1fr] grid-rows-[auto_1fr] min-h-screen w-full bg-black">
      ...
      <aside className="sticky top-0 self-start h-[calc(100vh-var(--spacing-sidebar-height))] overflow-y-auto p-12 py-10">
        {sidebar}
      </aside>
      <main className="overflow-y-auto border-l border-zinc-800 px-15 py-10">{children}</main>
    </div>
  )
}
